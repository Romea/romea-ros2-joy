...
protected:
    ...
    void SetUp() override
    {
        ...
        node = std::make_shared<rclcpp::Node>("test_joy_remapping", no);
...
TEST_F(TestJoyRemapping, testPartialRemapping)
{
  ...
  auto buttons_mappings=mapping.get(node_parameters->loadMap<int>("buttons.mapping"));
  ...
  auto sticks_mappings=mapping.get(node_parameters->loadMap<int>("axes.sticks.mapping"));
  ...
  auto triggers_mappings=mapping.get(node_parameters->loadMap<int>("axes.triggers.mapping"));
  EXPECT_EQ(triggers_mappings.at("speed"),5);
  EXPECT_EQ(triggers_mappings.at("LT"),2);

  auto dpad_mappings=mapping.get(node_parameters->loadMap<int>("axes.directional_pads.mapping"));
  EXPECT_EQ(dpad_mappings.at("trigger"),6);
  EXPECT_EQ(dpad_mappings.at("Vertical_Directional_Pad"),7);
}

TEST_F(TestJoyRemapping, testPartialRemappingKeepOnlyRemapped)
{
  std::map<std::string,std::string> joystick_remapping;
  joystick_remapping["B"]="stop";
  joystick_remapping["RT"]="speed";
