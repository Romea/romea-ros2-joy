...
TEST_F(TestJoyRemapping, checkPartialRemapping)
{
  ...
  auto buttons_mappings = remapping.apply(romea::extract_mappings(joystick_configuration["buttons"]));
  ...
  auto sticks_mappings=remapping.apply(romea::extract_mappings(joystick_configuration["axes"]["sticks"]));
  ...
  auto triggers_mappings=remapping.apply(romea::extract_mappings(joystick_configuration["axes"]["triggers"]));
  EXPECT_EQ(triggers_mappings.at("speed"),5);
  EXPECT_EQ(triggers_mappings.at("LT"),2);
  EXPECT_FALSE(remapping.is_complete());

  auto dpad_mappings=remapping.apply(romea::extract_mappings(joystick_configuration["axes"]["directional_pads"]));
